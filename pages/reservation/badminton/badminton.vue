<template>
	<view style="width: 100%;" v-if="show">
		...
		<button class="placeOrderButton" type="primary" @click="go_To_Paid('羽毛球')">生成订单</button><!-- 要改 -->
	</view>
...
<script setup>
	...
	const timeList = ss.badminton.timeList //要改！
	// timeField: ["t9", "t10", "t11", "t12", "t13", "t14", "t15", "t16", "t17", "t18", "t19", "t20", "t21"],
	const timeField = ss.badminton.timeField //要改！
	const timeNum = timeList.length
	const priceList = ss.badminton.priceList //要改！
	...
	uni.setStorageSync('changeVenueStateUrl', '/venueInfo/venueState/badminton/checkAndChangeVenueState/today')
	...
	function selectDate(e) {
		...
		if (e == 1) {
			...
			uni.setStorageSync('changeVenueStateUrl', '/venueInfo/venueState/badminton/checkAndChangeVenueState/tomorrow')
		}
	}
	...
	function go_To_Paid(venueType) {
		...
		if (count == 1 || count == 2) {
			...
			if (count == 2) {
				...
				let finishTime2
				for (let i = 0; i < timeNum; i++) {
					if (selectedState.value[secondSelectedVenue][i] == true) {
				...
				//创建第二个场地的订单细节
				let venue2 = gd.createOrderDetail.create_Order_Detail(secondSelectedVenue, time2, price2)
				let orderInfo = gd.createOrder.create_Order(timestamp, userID, '待支付', venueType, count, reservationDate,
					useDate, finishTime, venue1, venue2, false)
				uni.setStorageSync('orderInfo', orderInfo)
				uni.redirectTo({
