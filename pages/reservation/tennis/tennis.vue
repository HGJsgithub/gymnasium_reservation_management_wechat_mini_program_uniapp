	...
	<view style="width: 100%;" v-if="show1&&show2">
		...
		<view class="select-menu">
			<view class="time-area">
				<view style="text-align: center;padding-bottom: 20rpx;">网球</view>
		...
		<button class="placeOrderButton" type="primary" @click="go_To_Paid('网球')">生成订单</button><!-- 要改 -->
	</view>
...
<script setup>
	...
	const timeList = ss.tennis.timeList //要改！
	// timeField: ["t9", "t10", "t11", "t12", "t13", "t14", "t15", "t16", "t17", "t18", "t19", "t20", "t21"],
	const timeField = ss.tennis.timeField //要改！
	const timeNum = timeList.length
	const priceList = ss.tennis.priceList //要改！
	...
	//获取今天的场地状态
	uni.request({
		url: baseUrl + "/venueInfo/venueState/tennis/getVenueState/today",
		header: {
	...
	//获取明天的场地状态表
	uni.request({
		url: baseUrl + "/venueInfo/venueState/tennis/getVenueState/tomorrow",
		header: {
			"ngrok-skip-browser-warning": "true",
		},
		success: (res) => {
	...
	uni.setStorageSync('changeVenueStateUrl', '/venueInfo/venueState/tennis/checkAndChangeVenueState/today')
	...
	function selectDate(e) {
		...
		if (e == 1) {
			...
			uni.setStorageSync('changeVenueStateUrl',
				'/venueInfo/venueState/tennis/checkAndChangeVenueState/tomorrow')
		}
	}
